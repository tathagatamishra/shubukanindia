...
const MouseTrail = () => {
  ...
  useEffect(() => {
    ...
    const circlesNum = 100; // Fewer circles for better performance with filters
    const easing = 0.15;
    ...
    const render = () => {
      ...
      if (distance > 0) {
        ...
        for (let i = 0; i < pointsToAdd; i++) {
          ...
          const interpX = prevPositionRef.current.x + ratio * (positionRef.current.x - prevPositionRef.current.x);
          const interpY = prevPositionRef.current.y + ratio * (positionRef.current.y - prevPositionRef.current.y);
